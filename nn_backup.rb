...
# OPTIMIZE - Implement compression and retention of backup files
#
...
def download_fsimage (namenode_host, namenode_port, hadoop_version, download_dirs)
  ...
  begin
    ...
    download_dirs.each do |dir|
      FileUtils.cp(file_to_write, dir)
    end
  rescue Timeout::Error
    puts "The request for a page at #{uri} timed out...exiting."
    exit
...
def download_edits (namenode_host, namenode_port, hadoop_version, start_trans_id, end_trans_id, download_dirs)
  if hadoop_version =~ "1.[0-9].[0-9]"
...
download_fsimage(options[:namenode], options[:port], hadoop_version, options[:dirs])
download_edits(options[:namenode], options[:port], hadoop_version, options[:start_trans_id], options[:end_trans_id], options[:dirs])